#!/usr/bin/env python3
"""Aplica os personagens v4 (assets do pixel-agents) + baias ao viewer/index.html da main. Falha por assert se a base nao for a esperada."""
import sys,os
here=os.path.dirname(os.path.abspath(__file__))
p=sys.argv[1]
s=open(p,encoding="utf-8").read()
block=open(os.path.join(here,"pixelblock_v4.js")).read().replace("__V4DATA__",open(os.path.join(here,"v4data.js")).read().rstrip())
...
'''const DESK=s=>({x:s.x-DESK_W/2,y:s.y+CARD_H/2+4});
...
rep("const CARD_W=176,COLW=240,X0=620,MIDY=340,ROWGAP=232;","const CARD_W=176,COLW=240,X0=620,MIDY=340,ROWGAP=270;")
...
const BAY_COLS=3,BAY_DX=DESK_W+20,BAY_DY=STATION_H+22;
...
function startType(t,afk){
  ...
  const tick=()=>{
    ...
    f=1-f;frame(t,(t._read?"read":"type")+f);
    let d=afk?520:300;
    /* AFK: a cada rajada, chance de pausar com as maos na mesa */
    if(afk&&f===0&&Math.random()<.35){frame(t,(t._read?"read":"type")+"0");d=3000+Math.random()*4000;}
...
/* ocupacao das estacoes: PC aceso onde alguem esta sentado (mesa do passo ou baia) */
setInterval(()=>{
  const seated=Object.values(toks).filter(t=>t.pose==="sit"||t.pose==="sitback")
    .map(t=>({x:parseFloat(t.el.style.left),y:parseFloat(t.el.style.top)}));
  for(const pc of PCS){if(!pc._seat||!pc.isConnected)continue;
    pcOn(pc,seated.some(p=>Math.abs(p.x-pc._seat.x)<3&&Math.abs(p.y-pc._seat.y)<3));}
},420);''')
...
'''    d.innerHTML=deskSvg();
    stage.appendChild(d);deskEl[s.id]=d;
    /* PC em cima da mesa (tela apaga/acende com a ocupacao) e encosto da cadeira NA
       FRENTE do personagem — e o encosto que faz "sentar" ler como sentar */
    const pc=mkPc();pc.className="furn pc";pc.style.left=(DESK(s).x+PC_DX)+"px";pc.style.top=(DESK(s).y+PC_DY)+"px";pc.style.opacity=0;
    pc._seat=SEAT(s);stage.appendChild(pc);d._pc=pc;
...
s=s[:a]+'''  /* baias: piso discreto + 9 mesas em 3x3 (+ extras do roster, se houver) */
  ...
  for(let i=0;i<nb;i++){const p=BAYPOS(i);
    ...
    const pc=mkPc();pc.className="furn pc";pc.style.left=(p.x+PC_DX)+"px";pc.style.top=(p.y+PC_DY)+"px";pc._seat={x:p.x+SIT_DX,y:p.y+SIT_DY};stage.appendChild(pc);
...
'''    if(deskEl[s.id]){gsap.to(deskEl[s.id],{opacity:1,duration:dur,delay:liveMode?i*.07:0});
      ...
      if(deskEl[s.id]._pc)gsap.to(deskEl[s.id]._pc,{opacity:1,duration:dur,delay:liveMode?i*.07:0});}});''')
...
'''        const d=deskEl[s.id];
        if(d){d.style.left=DESK(s).x+"px";d.style.top=DESK(s).y+"px";
          ...
          if(d._pc){d._pc.style.left=(DESK(s).x+PC_DX)+"px";d._pc.style.top=(DESK(s).y+PC_DY)+"px";d._pc._seat=SEAT(s);}}});''')
...
.furn.pc{z-index:3}     /* PC em cima da mesa, atras do personagem */''')
s=s.replace('natv:"na tv",','natv:"na baia",',1).replace('natv:"watching tv",','natv:"at their bay",',1)
open(p,"w",encoding="utf-8").write(s); print("patch v4 aplicado em",p)
